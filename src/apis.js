...
export const getAPIAuthHeaders = api_key => {
  let authHeaders = new Headers()
  authHeaders.set('Content-Type', 'application/json')
  authHeaders.append('api-key', api_key || APIKEY)
  return authHeaders
}
...
export const getMessageQueue = apiKey => {
  const authHeaders = getAPIAuthHeaders(apiKey)
  let opts = {
    method: 'GET',
    headers: authHeaders
  }
  return fetch(`${env.API_BASE}/api/app/messaging/queue`, opts)
}

export const deleteMessageQueue = apiKey => {
  const authHeaders = getAPIAuthHeaders(apiKey)
  let opts = {
    method: 'DELETE',
    headers: authHeaders
...
export const postNewBulkToSubs = (apiKey, data) => {
  const authHeaders = getAPIAuthHeaders(apiKey)
  let opts = {
    method: 'POST',
    body: JSON.stringify(data),
  ...
  return fetch(`${env.API_BASE}/api/app/messaging/send_bulk_to_subs`, opts)
}

export const sendSmsWithoutCharge = (apiKey, data) => {
  const authHeaders = getAPIAuthHeaders(apiKey)
  let opts = {
    method: 'POST',
    body: JSON.stringify(data),
